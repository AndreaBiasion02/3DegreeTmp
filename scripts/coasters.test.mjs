...
test('Sixteen coasters preserve the requested phrases and have high-resolution images',async()=>{
 ...
 assert.deepEqual(products.map(p=>p.name),expected);assert(products.every(p=>/NFC integrato/.test(p.description)&&/NFC integrato/.test(p.assembly)));
...
test('Coasters are 100 mm wide, 4 mm thick, with a flush solid 0.6 mm graphic',()=>{
 for(const p of products){const b=fs.readFileSync('public'+p.model),l=b.readUInt32LE(12),j=JSON.parse(b.subarray(20,20+l)),bin=b.subarray(28+l);const bounds=[[Infinity,-Infinity],[Infinity,-Infinity],[Infinity,-Infinity]];
  ...
  assert.deepEqual(bounds,[[-50,50],[-50,50],[0,4]]);assert.deepEqual(p.dimensions,[100,100,4]);
 }
});
