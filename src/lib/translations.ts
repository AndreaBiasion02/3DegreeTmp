const dictionary: Record<string, string> = {
  // Nav / Layout
  Home: "Home",
  Store: "Store",
  Account: "Account",
  Cart: "Carrello",
  Menu: "Menu",
  Close: "Chiudi",
  Language: "Lingua",
  Country: "Paese",
  "All rights reserved": "Tutti i diritti riservati",
  Back: "Indietro",
  "Back to shopping cart": "Torna al carrello",
  Search: "Cerca",
  "Announcement Bar":
    "Rendi indimenticabile il giorno della tua laurea",
  "Home Title": "Il tuo traguardo, in 3D",
  Featured: "In evidenza",
  "Previous products": "Prodotti precedenti",
  "Next products": "Prodotti successivi",
  "Discover collection": "Scopri la collezione",
  "Legal links": "Informazioni legali",
  "Hero Eyebrow": "Bomboniere di laurea in stampa 3D",
  "Hero Title Line 1": "Il tuo traguardo.",
  "Hero Title Lead": "Il tuo",
  "Hero Title Subject": "traguardo",
  "Hero Title Line 2": "In 3D",
  "Hero Description New":
    "Trasforma il giorno della laurea in un ricordo che parla davvero di te: scegli, personalizza e crea il tuo dettaglio unico.",
  "Create Yours": "Crea la tua",
  "How It Works": "Come funziona",
  "Made for graduation": "Pensato per la laurea, creato per restare",
  "Made in Italy": "Fatto in Italia",
  "Made to order": "Fatto su richiesta",
  "Personalized graduation favors": "Bomboniere personalizzate",
  "Hero Image Alt": "Bomboniere di laurea personalizzate 3Degree",
  "Hero Floating Card": "Ogni dettaglio può raccontare la tua storia",
  "Our tool": "Il nostro strumento",
  "Your detail": "Il tuo dettaglio",
  Customizable: "Personalizzabile",
  "3D printed": "Stampato in 3D",
  "Made with care": "Creato con cura",
  "Secure checkout": "Checkout sicuro",
  "Why trust us": "I valori di 3Degree",
  "Simple process": "Un processo semplice",
  "From idea to memory": "Dall'idea al ricordo",
  "Process Intro":
    "Scegli il ricordo che vuoi collegare alla tua bomboniera. Quando avvicini il telefono, si apre subito; se vorrai, potrai cambiarlo in qualsiasi momento.",
  "Process 1 Title": "Scegli il ricordo",
  "Process 1 Text":
    "Scegli una pagina o un album condiviso, come Google Foto, dove raccogliere le immagini della laurea.",
  "Process 2 Title": "Collegalo alla bomboniera",
  "Process 2 Text":
    "Dal tuo spazio personale scegli cosa deve aprirsi quando avvicini il telefono. Potrai sostituirlo quando vuoi.",
  "Process 3 Title": "Avvicina e condividi",
  "Process 3 Text":
    "Durante la festa, basta avvicinare uno smartphone alla bomboniera per aprire l'album e aggiungere nuove foto.",
  "Process 4 Title": "Rivivi ogni momento",
  "Process 4 Text":
    "Dopo la festa puoi continuare ad arricchire l'album. La bomboniera riporterà sempre al ricordo che hai scelto.",
  "Video caption": "Avvicina il telefono. Il tuo ricordo si apre.",
  "Video unsupported": "Il tuo browser non supporta il video HTML5.",
  "Start now": "Inizia ora",
  "Why 3Degree": "Perché 3Degree",
  "Not a favor": "Non una bomboniera\nLa tua",
  "Value Intro":
    "La tecnologia è il punto di partenza. Il risultato è un oggetto personale, curato e capace di ricordare un momento irripetibile.",
  "Value 1 Title": "Davvero personale",
  "Value 1 Text":
    "Non aggiungiamo solo un nome: costruiamo un dettaglio coerente con la tua storia.",
  "Value 2 Title": "Design intenzionale",
  "Value 2 Text":
    "Forme, colori e proporzioni sono pensati per essere belli il giorno della festa e dopo.",
  "Value 3 Title": "Produzione su misura",
  "Value 3 Text":
    "La stampa 3D ci permette di produrre ciò che scegli, con meno standardizzazione e più identità.",
  "Clear before creating": "Tutto chiaro, prima di creare",
  "FAQ 1 Question": "Cosa posso personalizzare?",
  "FAQ 1 Answer":
    "Le opzioni disponibili cambiano in base al modello e vengono mostrate direttamente nel configuratore del prodotto prima dell'acquisto.",
  "FAQ 2 Question": "Posso vedere il risultato prima dell'ordine?",
  "FAQ 2 Answer":
    "Sì. Nel configuratore puoi controllare le scelte disponibili e il riepilogo della personalizzazione prima di aggiungere il prodotto al carrello.",
  "FAQ 3 Question": "Come scelgo il prodotto giusto?",
  "FAQ 3 Answer":
    "Esplora le collezioni partendo dalla tua facoltà, dallo stile della festa o dal tipo di ricordo che vuoi lasciare agli invitati.",
  "Your milestone": "Il tuo momento è arrivato",
  "Make it yours": "Rendilo indimenticabile",
  "Explore collection": "Esplora la collezione",
  "Footer Description New":
    "Bomboniere di laurea personalizzate, progettate con cura e realizzate attraverso la stampa 3D.",
  "Footer Help":
    "Gestisci ordini, indirizzi e dati del tuo profilo in un unico spazio.",
  "Your account": "Il tuo account",
  "Designed for milestones": "Progettato per i traguardi che contano",
};
Object.assign(dictionary, {
  "Hero Description New":
    "Trasforma il giorno della laurea in un ricordo che parla davvero di te: scopri le bomboniere 3Degree, tra design, personalità e stampa 3D.",
  "Create Yours": "Scopri i modelli",
  "Made to order": "Creato con cura",
  "Secure checkout": "Design italiano",
  "Start now": "Esplora le collezioni",
  "Process 1 Text":
    "Scegli una pagina o un album condiviso, come Google Foto, dove raccogliere le immagini della laurea.",
  "Process Intro":
    "Scegli il ricordo che vuoi collegare alla tua bomboniera. Quando avvicini il telefono, si apre subito; se vorrai, potrai cambiarlo in qualsiasi momento.",
  "Process 2 Text":
    "Dal tuo spazio personale scegli cosa deve aprirsi quando avvicini il telefono. Potrai sostituirlo quando vuoi.",
  "FAQ 1 Question": "Posso acquistare da questo sito?",
  "FAQ 1 Answer":
    "Questo sito è una vetrina illustrativa: presenta i modelli e le loro caratteristiche, senza vendita online diretta. Per richiedere informazioni, disponibilità o preventivi per la tua laurea, scrivici nei DM su Instagram (@3degree_lab), TikTok (@3degreelab) oppure via email a info@3degreelab.com.",
  "FAQ 2 Question": "Cosa mostrano le anteprime?",
  "FAQ 2 Answer":
    "Le immagini e le viste 3D mostrano modelli digitali. Forme di laurea e Tocchi di facoltà si esplorano aperti e chiusi, cambiando i colori. Nella collezione Personalizzabili puoi aggiungere testi e logo. Colori e finiture fisiche possono differire.",
  "FAQ 3 Question": "Come scelgo il modello da esplorare?",
  "FAQ 3 Answer":
    "Esplora i portaconfetti a tema oppure scegli un tocco per facoltà. Per una composizione libera, apri Costruisci la tua bomboniera. Ogni configurazione è solo illustrativa e non richiede un account.",
  "Clear before creating": "Conosci ogni dettaglio",
});
export const t = (key: string) => dictionary[key] ?? key;
